...
@Controller
public class ServiceChartCtrl {
	private ServiceChartSvc serviceChartSvc;

	public void setServiceChartSvc(ServiceChartSvc serviceChartSvc) {
		this.serviceChartSvc = serviceChartSvc;
		
	}
	
	@GetMapping("/serviceChart")	// 모집 목록
	public String serviceChart(Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (loginInfo == null) {
			...
			out.println("location.href='login?url=serviceChart';");
			out.println("</script>");
			out.close();
		}
		...
		rcnt = serviceChartSvc.getServiceInfoCount(where);
		
		List<ServiceChartInfo> serviceChartInfo = serviceChartSvc.getServiceChartInfo(where, cpage, psize);	// jdbc템플릿이 제공하는 것이 List여서 List 사용하는 것임 리미트 절에서 사용해야 하기 때문에 cpage, psize 가져감 아니면 where 절처럼 만들어서 사용가능
		...
		PageInfo pi = new PageInfo();
		 ...
		 model.addAttribute("serviceChartInfo", serviceChartInfo);	// model에 담아 보내기
		 model.addAttribute("pi", pi);

		return "service/serviceChart";
	}
}
