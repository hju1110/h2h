...
public class ServiceRequestListCtrl {
	private ServiceRequestListSvc serviceRequestListSvc;
		...
		public String serviceChart(Model model, HttpServletRequest request) throws Exception {
			...
			String where = " WHERE a.si_view = 'y' "; 
			String args = "";	
			
			args = "&cpage=" + cpage;	// 앞에 &가 있다는 것은 얘 앞에 뭔가가 또 올거라는 이야기 , 처음에 작성할 때 잘 생각해서 만들기
			...
			List<ServiceMember> serviceRequestListInfo = serviceRequestListSvc.getServiceRequestListInfo(where, miid, cpage, psize);	// jdbc템플릿이 제공하는 것이 List여서 List 사용하는 것임 리미트 절에서 사용해야 하기 때문에 cpage, psize 가져감 아니면 where 절처럼 만들어서 사용가능
