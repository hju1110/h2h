...
public class OrderProcDao {
	...
	public int orderProcIn(OrderProcInCtrl oi, String pi_id, int cnt) {
		String sql = "insert into t_order_info values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'a', now())";
			...
			sql = "UPDATE t_product_info SET pi_stock = pi_stock - "+cnt+", pi_sale = pi_sale + "+cnt+" WHERE pi_id = '" + pi_id + "' ";
			result += jdbc.update(sql);
			//System.out.println(sql);
	...
	public int productLiek(String pi_id) {
		String sql = "UPDATE t_product_info SET pi_good = pi_good + 1 WHERE pi_id = '" + pi_id + "' ";
