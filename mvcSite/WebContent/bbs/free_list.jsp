...
int pcnt = pageInfo.getPcnt();		int psize = pageInfo.getPsize();
int rcnt = pageInfo.getRcnt();		String schtype = pageInfo.getSchtype();
String keyword = pageInfo.getKeyword(),	args = pageInfo.getArgs();	String schargs = pageInfo.getSchargs();
%>
<style>
#list tr { height:30px; }
...
<tr><td align="right">
	...
	<input type="button" value="전체 글보기" onclick="location.href='freeList'" />
	</fieldset>
	</form>
</td></tr>
</table> 
...
<tr>
<th width="10%">번호</th><th width="*">제목</th><th width="15%">작성자</th>
...
if (freeList.size() > 0) {
	...
	for (FreeList fl : freeList) {
		...
		if (fl.getFl_reply() > 0) 	title += " [" + fl.getFl_reply() + "]";
%>
	...
	if (cpage == 1) {
	 ...
	 int spage = (cpage - 1) / bsize * bsize + 1;    // 현재 블록에서의 시작 페이지 번호 
	   for (int i = 1, j = spage ; i <= bsize && j <= pcnt; i++,j++) {
		...
		// j : 실제 풀력할 페이지 번호로 전체 페이지 개수(마지막 페이지 번호 )를 넘지 않게 사용해야 함 
	      if (j == cpage) {
