...
// 비회원글 : 모두에게 수정, 삭제 버튼을 보야줌 - 클릭시 비밀번호 입력폼으로 이동
...
if (freeList.getFl_ismem().equals("n")) {		// 현재 게시글이 비회원 글이면 
	...
	// 현재 사용자가 로그인이 되어 있고, 현 게시글의 작성자일 경우 
	isPms = true;
...
function realDel () {
	...
	<input type="button" value="글수정" onclick="<%=upLink %>;"/>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="button" value="글삭제" onclick="<%=delLink %>;" />
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<%} %>	
...
.frmUp { display:none; }
.reWriter { width:700px; display:flex; padding:5px; justify-content:space-between; border:1px solid black; background-color:#efefef;}
...
function replyIn() {		// ajax를 이용한 댓글 등록 함수
	...
	if (fr_content != "") {
		$.ajax({
			type : "POST", url : "/mvcSite/freeReplyProcIn", 
			data : {"flidx" : "<%=flidx %>", "fr_content" : fr_content},
			success : function(chkRs) {
				if (chkRs == 0) {
					alert("댓글 등록에 실패했습니다. \\n 다시 시도해 보세요.");
				} else {
					location.reload();		// 값이 있으면 새로고침 해서 그 값을 보여줌 
				}
			}
		});
	} else {
...
function replyGnb(gnb, fridx) {	// ajax를 이용한 댓글좋아요 싫어요 등록 함수
	$.ajax({
		type: "POST", url : "/mvcSite/freeReplyProcGnb",
		data : {"gnb" : gnb, "fridx" : fridx},
		success : function(chkRs) {
			if (chkRs == 2) {
				location.reload();
			} else if (chkRs == -1) {
...
// 댓글 번호를 받아 해당 댓글을 삭제하는 함수 
	if (confirm(" 정말 삭제하시겠습니까?")) {
		$.ajax({
			type: "POST", url : "/mvcSite/freeReplyProcDel",
			data : {"flidx" : <%=flidx %>, "fridx" : fridx},
			success : function(chkRs) {
				if (chkRs == 0) {
					alert("처리에 실패했습니다.\n 다시 시도해 보세요.");
				} else {
...
if (replyList.size() > 0) {					// 보여줄 댓글 목록이 있으면
	for (FreeReply fr : replyList) {
		...
		boolean isDel = false;
		if (isLogin && loginInfo.getMi_id().equals(fr.getMi_id())) {	 //순서를 신경 써야 함 
		// 현재 로그인한 회원이 현 댓글을 입력한 회원이면 (삭제 권한이 있음 )
			isDel = true;
		}
%>
<div class="reWriter">
	<%=fr.getMi_id() %>&nbsp;&nbsp;&nbsp;&nbsp;<%=fr.getFr_date() %>
	<img src="img/thumbs-up.png" width="20" onclick="<%=lnkG %>" class="hand" style="margin-left:400px;" />
	<%=fr.getFr_good() %>
	<img src="img/thumbs-down.png" width="20" onclick="<%=lnkB %>" class="hand" style="margin-left:10px;"/>
	<%=fr.getFr_bad() %>
	<% if (isDel) { %>
	<img src="img/close.png" width="20" class="hand" onclick="replyDel(<%=fridx %>);" />
	<% } %>
</div>
<div class="reContent"><xmp><%=fr.getFr_content() %></xmp></div>
<% 		
	}
} else {
