...
// 장바구니네 특정 삼품의 옵션 밒 수량을 변경하는 함수 	
// cnt가 0이면 옵션 변경이고 , opt가  0 이면 수량 변경을 의미
	$.ajax ({
...
// 장바구니네 특정 상품을 삭제하는 함수 
	if (confirm("정말 삭제하시겠습니까?")) {
		$.ajax ({
			type : "POST", url : "/mvcSite/cartProcDel",
			data : {"ocidx" : ocidx},
			success : function(chkRs) {
				if (chkRs == 0) {		
					alert("상품 삭제에 실패했습니다. \n 다시 시도하세요.");
...
<td width="15%">
	...
	<img src="/mvcSite/product/pdt_img/<%=oc.getPi_img1() %>" width="100" height="90" /></a>
</td>
<td width="20%">
...
<%
		int max = 10;			// 수량 변경을 위한 최대값을 지정할 변수
		...
		for(ProductStock ps : stockList) {
			...
			if (ps.getPs_idx() == oc.getPs_idx()) { 
				...
				max = ps.getPs_stock(); // 사용자가 선택한 옵션의 재고량을 max에 저장
				if (max > 10)	max = 10;
			}
...
<td width="15%">
	가격 : <%=oc.getPi_price() * oc.getOc_cnt() %>
</td>
<td width="*%">
