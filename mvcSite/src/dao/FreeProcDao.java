...
// 자유게시판 관련 쿼리 작업(등록 목록 수정 삭제)등 모두 처리하는 클래스 
		...
		// 자유게시판에서 검색된 결과의 레코드(게시글)목록을   ArrayList<FreeList>로 리턴하는 메소드 
			Statement stmt = null;
			ResultSet rs = null;
			...
			try {				
				String sql = "select fl_idx, fl_ismem, fl_writer, fl_title, fl_reply, fl_read, fl_ip, " + 
						"if (curdate() = date(fl_date),  right(fl_date , 8), replace(mid(fl_date, 3, 8), '-', '.')) wdate " + 
						"from t_free_list " + where +
	   ...
	   public int freeInsert(FreeList freeList) {
			   // 자유게시판 게시글 등록 처리 메소드 (글번호를 리턴), Dao는 디비작업임 무조건 try catch로 작업하기 예외가 있기 때문에
			      Statement stmt = null;
			      ResultSet rs = null;
			      int result = 0, flidx = 1;
			      
			      try {
			         stmt = conn.createStatement();
			         String sql = "select max(fl_idx) + 1 from t_free_list;";
			         rs = stmt.executeQuery(sql);
			         if (rs.next()) flidx = rs.getInt(1);
			         // 새롭게 등록할 게시글의 글번호를 생성하여 flidx에 저장
			         
			         sql = "insert into t_free_list (fl_idx, fl_ismem, fl_writer, fl_pw, fl_title, fl_content, fl_ip) value (" +
			         flidx + ", '" + freeList.getFl_ismem() + "', '" + freeList.getFl_writer() + "', '" + freeList.getFl_pw() + "', " +
			               " '" + freeList.getFl_title() + "', '" + freeList.getFl_content() + "', '" + freeList.getFl_ip() + "')";
			         System.out.println(sql);
			         result = stmt.executeUpdate(sql);
			         if (result == 0)   return 0;
			         // 오류 발생 시 글번호가 아닌 0을 리턴하여 svc에서 롤백을 시킴
			         
			      } catch(Exception e) {
			         System.out.println("FreeProcDao 클래스의 FreeInsert() 메소드 오류");
			         e.printStackTrace();   //어떤 종류의 오류인지 보기위해서
			      } finally {
			         close(rs); close(stmt);
			      }			      
			      return flidx;
	...
	// 수정할 특정 게시글의 정보들을 FreeList형 인스턴스로 받아 수정작업을 처리하는 메소드 
		...
		try {
			stmt = conn.createStatement();
			String sql = "update t_free_list set " + 
					"fl_title = '" + freeList.getFl_title() + "' , fl_content = '" + freeList.getFl_content() +  "' " + 
					" where fl_idx = " + freeList.getFl_idx();
			result = stmt.executeUpdate(sql);
		} catch(Exception e) {
